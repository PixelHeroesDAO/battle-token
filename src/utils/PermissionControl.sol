...
abstract contract PermissionControl is PermissionControlBase{
    ...
    */
    ...
    /// @dev Initializes the admin directly without authorization guard.
    /// This function must be called upon initialization,
    /// regardless of whether the contract is upgradeable or not.
    /// This is to enable generalization to both regular and upgradeable contracts,
    /// and to save gas in case the initial admin is not the caller.
    /// For performance reasons, this function will not check if there
    /// is an existing admin or `newAdmin` has any roles.
    function _initializeAdmin(address newAdmin) internal virtual {
        /// @solidity memory-safe-assembly
        assembly {
            mstore(0x0c, _ROLE_SLOT_SEED)
            mstore(0x00, newAdmin)
            // Store the new value.
            sstore(keccak256(0x0c, 0x20), ADMIN_ROLE)
            // Emit the {RolesUpdated} event.
            log3(0, 0, _ROLES_UPDATED_EVENT_SIGNATURE, shr(96, mload(0x0c)), ADMIN_ROLE)
        }
    }
